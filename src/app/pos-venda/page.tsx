...
interface Visita {
  ...
  hora: string; data_visita: string
}
...
interface Contato {
  id?: string; nome: string; telefone: string; email: string; cargo: string; cargo_outro: string
}
interface NegItem { valor: string; obs: string; concorrente: string; concorrente_outro: string }
interface NegEmpItem { func: string; vlr: string; obs: string; concorrente: string; concorrente_outro: string }
...
const CARGOS = ['Proprietário','Financeiro','Comercial','Gerente','Outros']
const CONCORRENTES = ['Pluxee','Ticket','VR','Caju','Flash','iFood','Alelo','Outros']

const PRODS_COM = [
  {id:21,nome:'Credenciamento',icon:'🏪',desc:'Credenciar na rede Vegas',   tipo:'taxa',   temConc:false},
  {id:22,nome:'Clube Vegas',   icon:'🎁',desc:'Programa de vantagens',      tipo:'livre',  temConc:false},
  {id:23,nome:'Cash Clube',    icon:'💰',desc:'Cashback e benefícios',      tipo:'taxa',   temConc:false},
  {id:24,nome:'Vegas Pay',     icon:'💳',desc:'Solução de pagamentos Vegas', tipo:'taxa',   temConc:false},
]
const PRODS_EMP = [
  {id:1, nome:'Alimentação',      icon:'🍽',cat:'Benefícios',tipo:'beneficio',desc:'Benefício alimentação',   temConc:true},
  {id:2, nome:'Refeição',         icon:'🥘',cat:'Benefícios',tipo:'beneficio',desc:'Benefício refeição',      temConc:true},
  {id:3, nome:'Aux. Farmácia',    icon:'💊',cat:'Benefícios',tipo:'beneficio',desc:'Aux. farmácia',           temConc:true},
  {id:4, nome:'Aux. Combustível', icon:'⛽',cat:'Benefícios',tipo:'beneficio',desc:'Aux. combustível',        temConc:true},
  {id:5, nome:'Farmácia Convênio',icon:'🏥',cat:'Convênio',  tipo:'beneficio',desc:'Convênio farmácias',      temConc:true},
  {id:6, nome:'Day Bank',         icon:'🏦',cat:'Convênio',  tipo:'beneficio',desc:'Conta digital',           temConc:false},
  {id:7, nome:'Combustível Frota',icon:'🚛',cat:'Convênio',  tipo:'beneficio',desc:'Gestão de frota',         temConc:false},
  {id:8, nome:'WellHub',          icon:'🏋',cat:'Agregado',  tipo:'agregado', desc:'Plataforma de bem-estar', temConc:false},
  {id:9, nome:'Total Pass',       icon:'🎯',cat:'Agregado',  tipo:'agregado', desc:'Acesso a academias',      temConc:false},
  {id:10,nome:'Telemedicina',     icon:'🩺',cat:'Agregado',  tipo:'agregado', desc:'Consultas médicas online',temConc:false},
  {id:11,nome:'Vidalink',         icon:'💊',cat:'Agregado',  tipo:'agregado', desc:'Auxílio Farmácia',        temConc:false},
  {id:12,nome:'Seguro MAC+Auto',  icon:'🚗',cat:'Agregado',  tipo:'agregado', desc:'Seguro de vida + auto',   temConc:false},
]
...
function novoContato():Contato{return{nome:'',telefone:'',email:'',cargo:'',cargo_outro:''}}
...
export default function PosVendaPage(){
  ...
  const [abaDetalhe,setAbaDetalhe]=useState<'negociacao'|'historico'|'contatos'>('negociacao')
  const [historico,setHistorico]=useState<HistoricoItem[]>([]),[carregandoHist,setCarregandoHist]=useState(false)
  const [contatos,setContatos]=useState<Contato[]>([novoContato()])
  const [contatosSalvos,setContatosSalvos]=useState<Contato[]>([]),[svContatos,setSvContatos]=useState(false)
  ...
  useEffect(()=>{
    if(!de||!csel)return
    if(abaDetalhe==='historico')carregarHistorico(csel.nome_fantasia)
    if(abaDetalhe==='contatos')carregarContatos(csel.id)
  },[de,csel,abaDetalhe])
  ...
  async function carregarHistorico(nome:string){
    ...
    if(data&&!error){
      setHistorico(data.map((v:any)=>({
        id:v.id,resultado:v.resultado||'contato',
        observacao:v.observacao||'',data_visita:v.data_visita,
  ...
  async function carregarContatos(comercioId:string){
    const{data}=await sb.from('comercio_contatos').select('*').eq('comercio_id',comercioId).order('created_at')
    if(data&&data.length>0){
      setContatosSalvos(data.map((c:any)=>({id:c.id,nome:c.nome,telefone:c.telefone||'',email:c.email||'',cargo:c.cargo||'',cargo_outro:''})))
      setContatos(data.map((c:any)=>({id:c.id,nome:c.nome,telefone:c.telefone||'',email:c.email||'',cargo:c.cargo||'',cargo_outro:''})))
    } else {
  ...
  async function salvarContatos(){
    ...
    // Deleta os antigos
    await sb.from('comercio_contatos').delete().eq('comercio_id',csel.id)
    // Insere os novos
    if(validos.length>0){
      await sb.from('comercio_contatos').insert(validos.map(c=>({
        comercio_id:csel.id,
        nome:c.nome.trim(),
        telefone:c.telefone||null,
        email:c.email||null,
        cargo:c.cargo==='Outros'?c.cargo_outro||'Outros':c.cargo||null,
      })))
    }
    await carregarContatos(csel.id)
    setSvContatos(false)
  }

  function updContato(i:number,campo:keyof Contato,val:string){
    setContatos(prev=>prev.map((c,idx)=>idx===i?{...c,[campo]:val}:c))
  }
  ...
  // Filtro com origem
  const filtBase=ccd.filter(c=>{
    const mOr=origemFiltro==='todos'||(origemFiltro==='leads'&&c.tipo_origem==='lead')||(origemFiltro==='posvendas'&&c.tipo_origem!=='lead')
    return mOr
  })

  // Contador por status dentro do filtro de origem
  function contStatus(s:string):number{
    return filtBase.filter(c=>c.status_crm===s).length
  }

  const filt=filtBase.filter(c=>{
  ...
  function abrirDetalhe(c:Comercio){
    ...
    setAbaDetalhe('negociacao');setHistorico([]);setContatos([novoContato()]);setContatosSalvos([])
    setDe(true)
  }
  ...
  function updNegEmp(id:number,campo:keyof NegEmpItem,val:string){setNegEmp(prev=>({...prev,[id]:{...(prev[id]||{func:'',vlr:'',obs:'',concorrente:'',concorrente_outro:''}),[campo]:val}}))}
  function updNegCom(id:number,campo:keyof NegItem,val:string){setNegCom(prev=>({...prev,[id]:{...(prev[id]||{valor:'',obs:'',concorrente:'',concorrente_outro:''}),[campo]:val}}))}

  function resumoNegociacao():string{
    const p:string[]=[]
    psEmp.forEach(id=>{
      const pr=PRODS_EMP.find(x=>x.id===id);const n=negEmp[id]
      if(pr){
        const v=n?.func&&n?.vlr?`${n.func}func×R$${n.vlr}=${calcTotal(n.func,n.vlr)}/mês`:'selecionado'
        const conc=n?.concorrente?` [Conc: ${n.concorrente==='Outros'?n.concorrente_outro||'Outros':n.concorrente}]`:''
        const o=n?.obs?` (${n.obs})`:'';p.push(`${pr.nome}: ${v}${conc}${o}`)
      }
    })
    psCom.forEach(id=>{
      const pr=PRODS_COM.find(x=>x.id===id);const n=negCom[id]
      if(pr){const v=n?.valor?`${n.valor}${pr.tipo==='taxa'?'%':''}`:'-';const o=n?.obs?` (${n.obs})`:'';p.push(`${pr.nome}: ${v}${o}`)}
    })
    return p.join(' | ')
  }
  ...
  async function salvarNovoLead(){
    ...
    if(error){alert('Erro: '+error.message);setSvLead(false);return}
  ...
  async function salvar(){
    ...
    const{error}=await sb.from('visitas_campo').insert({
      vendedor_id:usr?.id,nome_fantasia:csel.nome_fantasia,
      tipo_estabelecimento:csel.categoria,latitude:gla??-22.7330,longitude:glo??-47.3340,
      resultado:rs,observacao:oc||null,data_visita:new Date().toISOString()
    })
    if(!error){
  ...
  const sel:any={...inpSm,cursor:'pointer'}
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100vh',fontFamily:"'Plus Jakarta Sans',sans-serif",overflow:'hidden'}}>
            ...
            <div style={{padding:'10px 14px',borderBottom:'1px solid #e8eaed',background:'#fff',flexShrink:0}}>
              ...
              <div style={{display:'flex',gap:6,marginBottom:7,flexWrap:'wrap',alignItems:'center'}}>
                {[{k:'todos',l:'Todos'},{k:'posvendas',l:'🏪 Pós-venda'},{k:'leads',l:'⭐ Leads'}].map(f=>(
                  <button key={f.k} onClick={()=>{setOrFiltro(f.k as any);setSf('')}} style={{padding:'5px 12px',borderRadius:20,fontSize:12,fontWeight:700,cursor:'pointer',border:`1.5px solid ${origemFiltro===f.k?'#2563eb':'#e8eaed'}`,background:origemFiltro===f.k?'#2563eb':'#f4f5f7',color:origemFiltro===f.k?'#fff':'#6b7280',fontFamily:'inherit'}}>{f.l}</button>
                ))}
              ...
              {/* Filtros status COM CONTADORES */}
              <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                {[
                  {k:'',l:'Todos',icon:''},
                  {k:'ativo',l:'Ativo',icon:'🔵'},
                  {k:'novo_lead',l:'Lead',icon:'⭐'},
                  {k:'em_negociacao',l:'Neg.',icon:'🔥'},
                  {k:'visita_realizada',l:'Visita',icon:'✅'},
                  {k:'ligar',l:'Ligar',icon:'📞'},
                  {k:'proposta_enviada',l:'Proposta',icon:'📄'},
                  {k:'problema',l:'Prob.',icon:'⚠️'},
                  {k:'sem_contato',l:'S.Contato',icon:'❄️'},
                  {k:'fechado',l:'Fechado',icon:'🏆'},
                ].map(f=>{
                  const cnt=f.k?contStatus(f.k):filtBase.length
                  const ativo=sf===f.k
                  return(
                    <button key={f.k} onClick={()=>setSf(f.k)} style={{padding:'4px 9px',borderRadius:20,fontSize:10.5,fontWeight:600,cursor:'pointer',border:`1.5px solid ${ativo?'#2563eb':'#e8eaed'}`,background:ativo?'#2563eb':'#f4f5f7',color:ativo?'#fff':'#6b7280',fontFamily:'inherit',display:'flex',alignItems:'center',gap:3}}>
                      {f.icon&&<span>{f.icon}</span>}
                      <span>{f.l}</span>
                      {cnt>0&&<span style={{background:ativo?'rgba(255,255,255,0.3)':'#e8eaed',color:ativo?'#fff':'#374151',fontSize:9.5,fontWeight:800,padding:'0px 5px',borderRadius:10,minWidth:16,textAlign:'center'}}>{cnt}</span>}
                    </button>
                  )
                })}
              </div>
      ...
      {de&&csel&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:1000,backdropFilter:'blur(3px)'}}>
          <div style={{background:'#fff',borderRadius:'22px 22px 0 0',width:'100%',maxWidth:640,maxHeight:'96vh',display:'flex',flexDirection:'column'}}>
            <div style={{width:38,height:4,background:'#d1d5db',borderRadius:2,margin:'11px auto 0',flexShrink:0}}/>

            {/* Header */}
            <div style={{padding:'12px 18px',borderBottom:'1px solid #e8eaed',display:'flex',alignItems:'flex-start',gap:12,flexShrink:0}}>
              <div style={{width:44,height:44,borderRadius:12,background:csel.tipo_origem==='lead'?'#f5f3ff':(CAT[csel.categoria]||CAT.outros).bg,display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,flexShrink:0}}>{csel.tipo_origem==='lead'?'⭐':(CAT[csel.categoria]||CAT.outros).icon}</div>
              <div style={{flex:1}}>
                <div style={{display:'flex',alignItems:'center',gap:7,flexWrap:'wrap'}}>
                  <div style={{fontSize:16,fontWeight:800,color:'#111827'}}>{csel.nome_fantasia}</div>
                  {csel.tipo_origem==='lead'&&<span style={{background:'#f5f3ff',color:'#8b5cf6',fontSize:9.5,fontWeight:700,padding:'2px 7px',borderRadius:20,border:'1px solid #c4b5fd'}}>LEAD</span>}
                </div>
                <div style={{fontSize:11,color:'#6b7280',marginTop:1}}>{csel.subgrupo} · {csel.endereco?.split(' - ')[0]}</div>
                <div style={{display:'flex',gap:6,marginTop:4,flexWrap:'wrap'}}>
                  {csel.telefone&&<span style={{background:'#f4f5f7',color:'#374151',fontSize:10,fontWeight:600,padding:'2px 7px',borderRadius:20}}>📞 {csel.telefone}</span>}
                  {csel.distancia!=null&&<span style={{background:'#f0fdf4',color:'#16a34a',fontSize:10,fontWeight:600,padding:'2px 7px',borderRadius:20}}>📍 {fd(csel.distancia)}</span>}
                  {csel.data_proximo_contato&&<span style={{background:'#fffbeb',color:'#d97706',fontSize:10,fontWeight:600,padding:'2px 7px',borderRadius:20}}>📅 {new Date(csel.data_proximo_contato).toLocaleDateString('pt-BR')}</span>}
                </div>
              </div>
            ...
            {/* Status */}
            <div style={{padding:'8px 18px',borderBottom:'1px solid #e8eaed',flexShrink:0}}>
              <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:4}}>
                {PO.map(s=>{const st=S(s),at=(csel.status_crm||'ativo')===s;return(
                  <button key={s} onClick={()=>asmn(csel.id,s)} style={{padding:'6px 3px',borderRadius:8,border:`2px solid ${at?st.cor:st.border}`,background:at?st.bg:'#f4f5f7',cursor:'pointer',fontFamily:'inherit',textAlign:'center'}}>
            ...
            {/* Abas — 3 agora */}
            <div style={{display:'flex',borderBottom:'1px solid #e8eaed',flexShrink:0,background:'#fff',overflowX:'auto'}}>
              {[{k:'negociacao',l:'📦 Negociação'},{k:'contatos',l:`👤 Contatos${contatosSalvos.length>0?` (${contatosSalvos.length})`:''}`},{k:'historico',l:`📋 Histórico (${historico.length})`}].map(t=>(
                <div key={t.k} onClick={()=>setAbaDetalhe(t.k as any)} style={{padding:'10px 16px',fontSize:12.5,fontWeight:600,cursor:'pointer',color:abaDetalhe===t.k?'#2563eb':'#6b7280',borderBottom:`2px solid ${abaDetalhe===t.k?'#2563eb':'transparent'}`,marginBottom:-1,whiteSpace:'nowrap'}}>{t.l}</div>
              ))}
            </div>
            ...
            <div style={{flex:1,overflowY:'auto',minHeight:0}}>
                ...
                <div style={{padding:'12px 18px 28px'}}>
                  ...
                  {csel.produtos_negociando&&csel.produtos_negociando.length>0&&(
                    <div style={{background:'#fffbeb',border:'1px solid #fcd34d',borderRadius:10,padding:'10px 13px',marginBottom:12}}>
                      <div style={{fontSize:10,fontWeight:700,color:'#92400e',marginBottom:4,textTransform:'uppercase',letterSpacing:'.5px'}}>🔥 Em negociação</div>
                      ...
                      {csel.obs_crm&&<div style={{fontSize:11.5,color:'#6b7280',marginTop:4,fontStyle:'italic'}}>"{csel.obs_crm}"</div>}
                    </div>
                  )}
                  ...
                  <div style={{marginBottom:12}}>
                    <div style={{...lbl,marginBottom:7}}>🏪 Produtos Vegas para este comércio</div>
                    ...
                    {psCom.length>0&&(
                      <div style={{background:'#eff6ff',border:'1px solid #dbeafe',borderRadius:9,padding:'10px 13px'}}>
                        <div style={{fontSize:11,fontWeight:700,color:'#1d4ed8',marginBottom:7}}>💰 Detalhes da negociação</div>
                        {psCom.map(id=>{
                          const p=PRODS_COM.find(x=>x.id===id);if(!p)return null
                          const n=negCom[id]||{valor:'',obs:'',concorrente:'',concorrente_outro:''}
                          return(
                            <div key={id} style={{marginBottom:10,paddingBottom:10,borderBottom:'1px solid #dbeafe'}}>
                              ...
                              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:7}}>
                                ...
                                <div>
                                  ...
                                  <input type="text" value={n.obs} onChange={e=>updNegCom(id,'obs',e.target.value)} placeholder="Ex: mensal..." style={inpSm}/>
                                </div>
                              </div>
                            </div>
                          )
                  ...
                  <div style={{marginBottom:12}}>
                    <div style={{...lbl,marginBottom:7}}>👥 Produtos para funcionários</div>
                    {['Benefícios','Convênio','Agregado'].map(cat=>(
                      <div key={cat} style={{marginBottom:10}}>
                        ...
                        {/* Detalhes produto selecionado */}
                        {PRODS_EMP.filter(p=>p.cat===cat&&psEmp.includes(p.id)).map(p=>{
                          const n=negEmp[p.id]||{func:'',vlr:'',obs:'',concorrente:'',concorrente_outro:''}
                          return(
                            <div key={p.id} style={{background:'#f0fdf4',border:'1px solid #86efac',borderRadius:8,padding:'10px 12px',marginTop:5}}>
                              <div style={{fontSize:11,fontWeight:700,color:'#15803d',marginBottom:7}}>{p.icon} {p.nome}</div>
                              ...
                              {/* CAMPO OBS */}
                              <div style={{marginBottom:p.temConc?7:0}}>
                                <label style={lbl}>Observação</label>
                                <input type="text" value={n.obs} onChange={e=>updNegEmp(p.id,'obs',e.target.value)} placeholder="Ex: começa em abril, incluir dependentes..." style={inpSm}/>
                              </div>
                              {/* CONCORRENTE — só para Benefícios/Convênio */}
                              {p.temConc&&(
                                <div>
                                  <label style={lbl}>Concorrente atual</label>
                                  <select value={n.concorrente} onChange={e=>updNegEmp(p.id,'concorrente',e.target.value)} style={sel}>
                                    <option value="">Selecionar concorrente...</option>
                                    {CONCORRENTES.map(c=><option key={c} value={c}>{c}</option>)}
                                  </select>
                                  {n.concorrente==='Outros'&&(
                                    <input type="text" value={n.concorrente_outro} onChange={e=>updNegEmp(p.id,'concorrente_outro',e.target.value)} placeholder="Qual concorrente?" style={{...inpSm,marginTop:5}}/>
                                  )}
                                </div>
                              )}
                            </div>
                  ...
                  <div style={{marginBottom:14}}><label style={lbl}>Observação geral</label><textarea rows={2} value={ob} onChange={e=>setOb(e.target.value)} placeholder="Ex: Gerente demonstrou interesse..." style={{...inp,resize:'none'}}/></div>
              ...
              {abaDetalhe==='contatos'&&(
                <div style={{padding:'12px 18px 28px'}}>
                  <div style={{fontSize:12,color:'#6b7280',marginBottom:14}}>Adicione os contatos deste comércio. Clique em + para adicionar mais.</div>
                  {contatos.map((c,i)=>(
                    <div key={i} style={{background:'#f4f5f7',border:'1px solid #e8eaed',borderRadius:11,padding:'12px 14px',marginBottom:10}}>
                      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
                        <div style={{fontSize:12,fontWeight:700,color:'#374151'}}>👤 Contato {i+1}</div>
                        {contatos.length>1&&<span onClick={()=>setContatos(prev=>prev.filter((_,idx)=>idx!==i))} style={{fontSize:18,color:'#dc2626',cursor:'pointer',padding:2}}>✕</span>}
                      </div>
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                        <div style={{gridColumn:'span 2'}}>
                          <label style={lbl}>Nome *</label>
                          <input type="text" value={c.nome} onChange={e=>updContato(i,'nome',e.target.value)} placeholder="Nome completo" style={inp}/>
                        </div>
                        <div>
                          <label style={lbl}>Telefone</label>
                          <input type="tel" value={c.telefone} onChange={e=>updContato(i,'telefone',e.target.value)} placeholder="(00) 00000-0000" style={inp}/>
                        </div>
                        <div>
                          <label style={lbl}>E-mail</label>
                          <input type="email" value={c.email} onChange={e=>updContato(i,'email',e.target.value)} placeholder="email@empresa.com" style={inp}/>
                        </div>
                        <div>
                          <label style={lbl}>Cargo</label>
                          <select value={c.cargo} onChange={e=>updContato(i,'cargo',e.target.value)} style={{...inp,cursor:'pointer'}}>
                        ...
                        {c.cargo==='Outros'&&(
                          <div>
                            <label style={lbl}>Qual cargo?</label>
                            <input type="text" value={c.cargo_outro} onChange={e=>updContato(i,'cargo_outro',e.target.value)} placeholder="Ex: Sócio, TI..." style={inp}/>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <button onClick={()=>setContatos(prev=>[...prev,novoContato()])} style={{width:'100%',padding:'11px',background:'#fff',border:'2px dashed #2563eb',borderRadius:10,fontSize:13,fontWeight:700,color:'#2563eb',cursor:'pointer',fontFamily:'inherit',marginBottom:14,display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
                    ＋ Adicionar outro contato
                  </button>
                  <button onClick={salvarContatos} disabled={svContatos} style={{width:'100%',padding:13,background:svContatos?'#86efac':'#2563eb',color:'#fff',border:'none',borderRadius:11,fontSize:14,fontWeight:800,cursor:svContatos?'not-allowed':'pointer',fontFamily:'inherit'}}>
                    {svContatos?'Salvando...':'💾 Salvar Contatos'}
                  </button>
                  {contatosSalvos.length>0&&(
                    <div style={{marginTop:16}}>
                      <div style={{fontSize:11,fontWeight:700,color:'#6b7280',textTransform:'uppercase',letterSpacing:'.5px',marginBottom:8}}>Contatos salvos</div>
                      {contatosSalvos.map((c,i)=>(
                        <div key={i} style={{background:'#f0fdf4',border:'1px solid #86efac',borderRadius:9,padding:'10px 13px',marginBottom:7,display:'flex',alignItems:'center',gap:10}}>
                          <div style={{width:34,height:34,background:'#dcfce7',borderRadius:9,display:'flex',alignItems:'center',justifyContent:'center',fontSize:16,flexShrink:0}}>👤</div>
                          <div style={{flex:1}}>
                            <div style={{fontWeight:700,fontSize:13,color:'#111827'}}>{c.nome}</div>
                            <div style={{fontSize:11,color:'#6b7280',marginTop:1}}>{c.cargo}{c.telefone?` · ${c.telefone}`:''}{c.email?` · ${c.email}`:''}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div style={{padding:'12px 18px 28px'}}>
                  {carregandoHist?(
                    ...
                    <div style={{padding:30,textAlign:'center',color:'#9ca3af'}}>
                      ...
                      <div style={{fontWeight:600,marginBottom:4}}>Nenhuma interação ainda</div>
                      <div style={{fontSize:12}}>As visitas aparecem aqui após serem salvas.</div>
                    </div>
                  ):(
                    <>
                      <div style={{fontSize:12,color:'#6b7280',marginBottom:12}}>{historico.length} interação{historico.length!==1?'ões':''} registrada{historico.length!==1?'s':''}</div>
                      {historico.map((h,i)=>(
                        <div key={h.id} style={{background:'#fff',border:'1px solid #e8eaed',borderRadius:12,padding:'12px 14px',marginBottom:9,borderLeft:`3px solid ${STATUS_COR[SA[h.resultado]]||'#e8eaed'}`}}>
                          <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:h.observacao?8:0}}>
                            ...
                            {i===0&&<span style={{background:'#eff6ff',color:'#2563eb',fontSize:9.5,fontWeight:700,padding:'2px 7px',borderRadius:10,flexShrink:0}}>Mais recente</span>}
                          </div>
                          {h.observacao&&<div style={{background:'#f4f5f7',borderRadius:8,padding:'8px 11px',fontSize:12,color:'#374151',lineHeight:1.5}}>{h.observacao}</div>}
                        </div>
                      ))}
                    </>
                  )}
              ...
              <>
                ...
                <div style={{padding:'12px 18px 28px'}}>
                  <div style={{borderRadius:9,padding:'9px 12px',display:'flex',alignItems:'center',gap:9,marginBottom:12,fontSize:12,fontWeight:600,background:gs==='ok'?'#f0fdf4':'#eff6ff',border:`1px solid ${gs==='ok'?'#86efac':'#dbeafe'}`,color:gs==='ok'?'#16a34a':'#2563eb'}}>
                    <span>{gs==='ok'?'📍':'⏳'}</span><div><div style={{fontWeight:700}}>{gs==='ok'?'✓ GPS capturado!':'Capturando...'}</div><div style={{fontSize:10,fontWeight:400}}>{gm}</div></div>
                  </div>
                  {!scLead?(
                    <>
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
                    ...
                    <div style={{textAlign:'center',padding:'20px 0 10px'}}>
                      ...
                      <div style={{background:'#f5f3ff',border:'1px solid #ddd6fe',borderRadius:9,padding:'9px 12px',fontSize:11.5,color:'#7c3aed',marginBottom:16}}>📍 GPS salvo · Visita registrada</div>
                ...
                {!sc?(
                  <>
                    ...
                    <div style={{padding:'12px 18px 28px'}}>
                      ...
                      <div style={{borderRadius:9,padding:'9px 12px',display:'flex',alignItems:'center',gap:9,marginBottom:12,fontSize:12,fontWeight:600,background:gs==='ok'?'#f0fdf4':'#eff6ff',border:`1px solid ${gs==='ok'?'#86efac':'#dbeafe'}`,color:gs==='ok'?'#16a34a':'#2563eb'}}>
                        <span>{gs==='ok'?'📍':'⏳'}</span><div><div style={{fontWeight:700}}>{gs==='ok'?'✓ Localização capturada!':'Capturando...'}</div><div style={{fontSize:10,fontWeight:400}}>{gm}</div></div>
                      </div>
